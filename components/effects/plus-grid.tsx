...
export interface PlusGridConfig {
	...
	/** Subtle drift while the wave is energized; stops when scrolling stops */
	ambientAmplitude: number;
	/** Drift speed */
	ambientSpeed: number;
	/** Idle star-twinkle opacity swing (0–1); fades out while the wave is live */
	twinkleAmplitude: number;
	/** Idle star-twinkle base speed (rad/s) */
	twinkleSpeed: number;
	cols?: number;
	rows?: number;
}
...
interface Particle {
	...
	opacityMod: number;
	/** Random twinkle offsets so stars never blink in unison */
	twinklePhase: number;
	twinkleSpeed: number;
}

interface WaveState {
	energy: number; // 0–1, decays each tick — raw target written by inputs
	direction: number; // +1 = scroll down, -1 = scroll up — raw target
	phase: number; // advances each tick
	/** Eased copies of energy/direction — the draw math reads only these, so
	 *  instant target writes (scroll spikes, autoPulse flips) swell and
...
export default function PlusGrid({
	...
	const drawRef = useRef<((info: FrameInfo) => void | false) | null>(null);
	...
	const loop = useAnimationLoop({
		...
		onFrame: (info) => (drawRef.current ? drawRef.current(info) : false),
	});
	...
	useEffect(() => {
		...
		const tick = ({ dt }: FrameInfo) => {
			...
			// frame below is genuinely static. All advances are dt-normalized to
			// 60Hz-equivalent steps so a 120Hz display doesn't run the sim at 2x.
			if (!isPaused) {
				const steps = dt * 60;
				wave.energy *= Math.pow(cfg.waveDecay, steps);
				time += dt;
				wave.renderEnergy +=
					(wave.energy - wave.renderEnergy) * (1 - Math.pow(0.92, steps));
				wave.renderDir +=
					(wave.direction - wave.renderDir) * (1 - Math.pow(0.95, steps));
				// Wave and drift clocks advance only with live energy — no scroll,
				// no motion. The twinkle clock (`time`) is the one that never rests.
				wave.phase += cfg.waveSpeed * steps * wave.renderEnergy;
				ambientPhase += cfg.ambientSpeed * steps * wave.renderEnergy;
			}
			...
			for (const p of particles) {
				...
				// Subtle drift, alive only while the wave carries energy
				const idleY =
				...
				const opacityDelta =
					Math.cos(spatialArg) * cfg.opacityVariance * wave.renderEnergy;

				// Idle star twinkle — two incommensurate sines per star keep the
				// blink irregular; quadratic fade hands over to the wave on scroll.
				const calm = 1 - wave.renderEnergy;
				const t = time * cfg.twinkleSpeed * p.twinkleSpeed;
				const twinkle =
					Math.sin(t + p.twinklePhase) *
					Math.sin(t * 0.37 + p.twinklePhase * 2.1) *
				...
				const opacity = Math.max(
					0.04,
					Math.min(0.95, cfg.baseOpacity * p.opacityMod + opacityDelta + twinkle),
				);
