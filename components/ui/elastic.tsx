...
/** Slot geometry. Depth 0 is the card in your hand. Opaque full-size faces hide most
 *  of the offset behind the scale shrink, so the y step runs wider than the registry
 *  original (14) to keep the stack legible at the bottom edge. */
...
export interface ElasticHandle {
	/** Throw the front card to the back, as a pointer flick would. */
	next: () => void;
}
...
export interface ElasticProps {
	...
	/** Imperative handle so a keyboard-reachable control can cycle the deck. */
	handleRef?: Ref<ElasticHandle>;
}
...
export default function Elastic({
	...
	// A programmatic flick for controls a pointer can't reach: same flight, same queue
	// rotation, aimed out to the right the way a natural throw usually leaves.
	useImperativeHandle(
		handleRef,
		() => ({
			next: () => {
		...
		[queue, reducedMotion, bounds, x, y],
	);
