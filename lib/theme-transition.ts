		...
		case "star": {
			...
			const starPolygon = (radius: number) => {
				...
				for (let i = 0; i < 5; i++) {
					...
					verts.push(
						vertex(cx + radius * Math.cos(outerA), cy + radius * Math.sin(outerA)),
					);
 ...
 * full-viewport overlay in the *outgoing* ground color, swaps the theme beneath
...
function runFallbackReveal(
	...
	const oldGround = getComputedStyle(document.documentElement)
	...
	overlay.style.background = oldGround || "#050505";
